...
pub struct Checkpoint {
    ...
    params: RunParams,
    pub completed: usize,
    pub spectral_pairs: Vec<(SpectralResult, SpectralResult)>,
...
pub fn save(
    output_dir: &str,
    params: &RunParams,
    spectral_pairs: &[(SpectralResult, SpectralResult)],
    ...
    let ckpt = Checkpoint {
        ...
        params: params.clone(),
        completed: spectral_pairs.len(),
        spectral_pairs: spectral_pairs.to_vec(),
...
/// 2. `RunParams` matches the current CLI arguments field-by-field
/// 3. Data lengths are consistent
pub fn load(output_dir: &str, expected: &RunParams) -> Result<Checkpoint, String> {
    ...
    if &ckpt.params != expected {
        ...
        if p.n_points != e.n_points { diffs.push(format!("N: {} vs {}", p.n_points, e.n_points)); }
        if p.seed_base != e.seed_base { diffs.push(format!("seed: {} vs {}", p.seed_base, e.seed_base)); }
        if p.epsilon_bits != e.epsilon_bits { diffs.push("epsilon differs".to_string()); }
        if p.tmax != e.tmax { diffs.push(format!("tmax: {} vs {}", p.tmax, e.tmax)); }
        if p.eigen_cutoff != e.eigen_cutoff { diffs.push(format!("eigen-cutoff: {} vs {}", p.eigen_cutoff, e.eigen_cutoff)); }
        if p.exec_mode != e.exec_mode { diffs.push(format!("mode: {} vs {}", p.exec_mode, e.exec_mode)); }
        return Err(format!("parameter mismatch: {}", diffs.join(", ")));
    }

    if ckpt.spectral_pairs.len() != ckpt.completed {
        return Err(format!(
            "data integrity: spectral_pairs.len()={} != completed={}",
            ckpt.spectral_pairs.len(), ckpt.completed
        ));
    }
    if ckpt.topo_vec.len() != ckpt.completed {
        return Err(format!(
            "data integrity: topo_vec.len()={} != completed={}",
            ckpt.topo_vec.len(), ckpt.completed
        ));
    }
