...
\begin{verbatim}
  ...
  ├─────────────────────────────────────────────────────────┤
  │  Measurements M1–M10              (measure/)             │
  │    Mass, half-life, modulo, vacuum pol., electroweak,     │
  │    decoherence, neutrino, PMNS, Higgs, Lagrangian        │
  ├─────────────────────────────────────────────────────────┤
...
\begin{longtable}{lp{9cm}}
  ...
  \modname{measure/}       & Ten observer measurements M1--M10, each in
                             \file{m01\_traversal.rs} through \file{m10\_lagrangian.rs} \\
...
\begin{itemize}
  \item $\Omega_{\text{dark}}/\Omega_{\text{vis}} =
        \sum(N - |\Phi|) / \sum |\Phi|$
  \item $Q_{\text{topo}} = \sum |\Phi|^2 / \sum N^2$
        (topological charge ratio, intrinsic)
  \item $\alpha_{\text{EM}} = Q_{\text{topo}} / (8\pi)$
...
\]
where $P(t)$ is the return probability after $t$ diffusion steps on the graph.
Centred finite differences are used for interior points; forward/backward
...
\func{compute\_eigen()} computes the full eigendecomposition of the graph
Laplacian $L = D - A$ via \code{nalgebra}.  The return probability is then
\[
  P(t) = \frac{1}{N}\sum_{k=0}^{N-1} e^{-\lambda_k t},
\]
where $\lambda_k$ are the eigenvalues of the normalised Laplacian.
Core walks restrict the sum to eigenvectors with support on core nodes.

\section{Tier 2: Monte Carlo Walkers}

Source: \file{phase3/walker.rs}

For $N >$ \code{eigen\_cutoff}, random walkers simulate diffusion.  The number
of walkers $W$ is set by the \textbf{Causal Resolution Theorem}:
\[
  W = \lceil (t_{\max} / \varepsilon)^2 \rceil,
\]
where $\varepsilon$ is the topological error tolerance (default 0.01) and
$t_{\max}$ is the maximum diffusion time (default 15).  At $\varepsilon = 0.01$,
$t_{\max} = 15$: $W = 2{,}250{,}000$ walkers.

Each walker starts at a random node, takes $t$ steps along random edges of the
symmetric CSR, and records whether it returns to its origin.  Return counts are
accumulated as \code{u64} integers (strict finitism).
...
\end{verbatim}
giving steps 1--30 (unit spacing) and 32, 34, \ldots, 100 (stride 2).
...
The measurement system provides ten independent physics observables.  Each
...
Tests the emergence of the Born rule from modular path-integral interference.
Walkers perform NTT paths and the probability distribution is compared to the
quadratic law:
\[
  P(d) = |K(s, d)|^2,
\]
where $K(s, d)$ is the modular propagator from source $s$ to detector $d$.
The double consistency (retarded $\times$ advanced) of the discrete path
integral naturally produces the Born rule without postulation.

Output: \file{decoherence.csv} and \file{born\_rule.csv} with measured vs.\
...
\begin{longtable}{lll}
  ...
  \midrule
  $\alpha_{\text{EM}}$ & M4 & $Q_{\text{topo}}/(8\pi)$ \\
...
\begin{longtable}{llp{8cm}}
  ...
  \flag{tmax} \code{<usize>}   & 15    & Maximum diffusion time \\
...
\begin{longtable}{lp{10cm}}
  ...
  \flag{measure-all}         & Enable all M1--M9 \\
  \bottomrule
\end{longtable}
...
\begin{result}
  ...
  \begin{itemize}
    ...
    \item The crossover occurs at $t \sim 5$--$10$ diffusion steps
...
\begin{result}
  Phase-coherence coupling ($N = 50{,}000$, $M = 25$):
  \begin{itemize}
    \item $Q_{\text{topo}} = \sum|\Phi|^2 / \sum N^2 \approx 0.06$--$0.08$
    \item $\alpha_{\text{EM}} = Q_{\text{topo}} / (8\pi) \approx 1/132$ at bare scale
    \item Running via planarity screening: $\alpha^{-1} \to 137$ at long distances
  \end{itemize}
\end{result}
...
\begin{result}
  ...
  \begin{itemize}
    \item $P(d) \propto |K(s,d)|^2$ verified to within $5\%$
    \item Fringe visibility $> 0.8$ at $t < 10$; decoheres for $t > 50$
  \end{itemize}
\end{result}
...
\begin{itemize}
  ...
  \item \textbf{Mass ratio convergence}: The cover-time ratios $m_\mu/m_e$ and
        ...
        $10\%$, use $N \geq 50{,}000$.
  \item \textbf{Spectral dimension plateau}: The $\dS \to 4$ plateau may not be
        reached at small $t_{\max}$.  Use $t_{\max} \geq 15$ for convergence.
