...
\end{axiom}
\textit{Meaning:} Causality flows. If my grandfather caused my father, and my father caused me, my grandfather is my causal ancestor.

\begin{axiom}[Acyclicity]
There is no $x$ such that $x \prec x$.
\end{axiom}
\textit{Meaning:} No closed timelike curves. You cannot kill your own grandfather. Time has a direction; it is not a loop.

\begin{axiom}[Local Finiteness]
For any two elements $x, z$, the number of elements ``between'' them is finite.
\[ |\{ y \in \mathcal{C} \mid x \prec y \prec z \}| < \infty \]
\end{axiom}
\textit{Meaning:} This is the discreteness condition. In a continuum, between any two points there are infinitely many points. Here, there is only a handful. This implies that nature has a finite resolution.
...
We need to change our perspective. Instead of thinking of spacetime as a fixed shape, think of it as a \textbf{probability distribution}.

Imagine a local patch of spacetime. We can parameterize the ``state'' of this patch by some coordinates $\theta^\mu$. The physics is described by the probability distribution of causal links $p(C|\theta)$.
...
\begin{theorem}[title={Chentsov, 1972}]
The Fisher metric is the \emph{only} metric on the space of probability distributions that is invariant under sufficient statistics.
\end{theorem}

\begin{tcolorbox}[colback=mainlight,colframe=main,title=Note on the Derivation]
Standard Causal Set Theory typically recovers the manifold via the ``Number-Volume Correspondence'' directly (see \cite{bombelli1987}). Our approach here, utilizing the Fisher Information Metric and Chentsov's Theorem \cite{chentsov1982}, is a distinctive feature of Modulo Synthesis, emphasizing the information-theoretic origin of Lorentzian geometry.
\end{tcolorbox}
...
\section{The Sorkin D'Alembertian}

Raphael Sorkin solved this by embracing the non-locality. The wave operator $\square \phi$ at point $x$ depends not just on neighbors, but on layers of ancestors deeper in the past.
...
The discrete wave operator is a weighted sum over these layers:
\begin{equation}
  \square \phi(x) = \frac{1}{\lP^2} \left( a_0 \phi(x) + a_1 \sum_{y \in L_1} \phi(y) + a_2 \sum_{y \in L_2} \phi(y) + \dots \right)
\end{equation}
...
But in 1995, Ted Jacobson showed they might be \textbf{thermodynamic} equations, like $PV = nRT$.

The logic flow:
\begin{enumerate}
    \item \textbf{First Law:} $\delta Q = T dS$.
    \item \textbf{Flux:} Matter flowing across the horizon carries heat $\delta Q$.
    \item \textbf{Geometry:} The horizon area changes $dS \propto dA$.
    \item \textbf{Curvature:} The way area changes is governed by spacetime curvature (Raychaudhuri equation).
\end{enumerate}
...
\begin{tcolorbox}[colback=thirdlight,colframe=third,title=Note on Dynamics]
We emphasize that gravity here is \textbf{emergent/thermodynamic} (following Jacobson \cite{jacobson1995}), not fundamental. We do not vary a fundamental action (like the Benincasa-Dowker action \cite{benincasa2010}) to get these equations. The dynamics arise from the statistical mechanics of the underlying discretum.
\end{tcolorbox}
...
\subsection{The Solution: $\Lambda$ as a Fluctuation}

Remember Modulo 1. The volume $V$ is defined by counting $N$. $V \approx N$.
But this is a Poisson process. The count fluctuates!
The standard deviation is $\sqrt{N}$.
So the volume has an uncertainty $\delta V \sim \sqrt{V}$.
