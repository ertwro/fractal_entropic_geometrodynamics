...
\subsection{The Fisher Sphere and Topological Chemistry}

The Fisher metric derived above has a remarkable specialisation when applied to the phase simplex of the causal prism. Each intermediate node in a belly of size $n$ carries a causal phase $\varphi \in \{+1, 0, -1\}$. The phase fractions $(p_+, p_0, p_-)$ parametrise a trinomial distribution on the simplex $\Delta^2 = \{(p_+, p_0, p_-) : p_i \ge 0,\;\sum p_i = 1\}$.

Applying the Fisher Information Metric (equation above) to this trinomial:
\begin{equation}
    ds^2_{\mathrm{Fisher}} \;=\; n \sum_{i \in \{+,0,-\}} \frac{(dp_i)^2}{p_i}
\end{equation}
Under the coordinate transformation $\theta_i = 2\sqrt{p_i}$, the constraint $\sum p_i = 1$ becomes $\sum \theta_i^2 = 4$, and the metric becomes the round metric on the positive octant of $S^2(\sqrt{n})$:
\begin{equation}
    ds^2 = n\,d\Omega^2_{\text{octant}}, \qquad K = \frac{1}{n}
\end{equation}
The Gaussian curvature $K = 1/n$ is the fundamental regulator:
\begin{itemize}
    \item \textbf{Large bellies} ($n \gg 1$): nearly flat sphere, many spherical harmonics $Y_l^m$ fit as eigenmodes of the Laplacian.
    \item \textbf{Small bellies} ($n \sim 3$): highly curved sphere, only the lowest harmonics survive.
\end{itemize}

The eigenvalue degeneracies of $\Delta_{S^2}$ give shell capacities $\sum_{l=0}^{k-1}(2l+1) = k^2$, which upon CPT doubling (each prism has an antiprism) yield:
\begin{equation}
    \text{Shell capacity} = 2k^2 \quad\Longrightarrow\quad 2,\; 8,\; 18,\; 32
\end{equation}
These are exactly the magic numbers of atomic shell structure. The orbital labels $s\,(l{=}0)$, $p\,(l{=}1)$, $d\,(l{=}2)$, $f\,(l{=}3)$ correspond to generation labels: $l = 0$ is uniform phase (Generation~1), $l = 1$ is biphasic (Generation~2), $l = 2$ is triphasic (Generation~3). The full development of this ``Topological Chemistry'' appears in Volume~II.
...
The spectral dimension measurement encounters topological defects (Causal Prisms) during the random walk. These defects are classified by their \textbf{topological generation} $g \in \{1,2,3\}$---a directed invariant defined by the causal phase partition of the belly intermediates. The formal definition and physical interpretation of generations are deferred to Volume~II, Modulo~10.

\stoneref{The generation number $g$ counts how many distinct logical polarities coexist within a defect---a measure of internal dialectical complexity in the Heyting algebra of the causal order \cite{stone1936}.}
...
\textbf{Finite-size scaling.} A systematic analysis across five lattice sizes ($N = 10^5$ to $10^7$) confirms the $N^{-1/4}$ boundary/volume scaling for $\mathcal{Q}_{\mathrm{topo}}(N)$ with $R^2 = 0.9996$. The full table of extrapolated parameters---including the bare coupling $\alpha_0^{-1} = 165.1$ and the thermodynamic-limit dark matter ratio $\Omega_\infty = 5.57$---is presented in Volume~II, Chapter~17 (Vacuum Polarization and the Running Coupling).
...
\subsection{Numerical Verification: The Factor of 4 from Graph Data}

The $N = 10^7$ simulation data provides the first numerical verification of
the Bekenstein--Hawking entropy coefficient from a discrete causal set.  Two
independent measurements of Newton's constant~$G$ are extracted from the raw
graph data:

\begin{enumerate}[leftmargin=*]
  \item \textbf{Thermodynamic route (Clausius flux):} Applying the Clausius
    relation $\delta Q = T\,\delta S$ to local causal horizons, with the
    physical area formulation $A = V^{1/2}$ (the $d = 4$ null horizon area),
    yields a constant ratio $\delta Q / (T\,\delta S) = 8\pi G$ with
    coefficient of variation $\mathrm{CV} = 26.1\%$.  This gives
    $G_{\mathrm{thermo}} = 0.231$ in link units.
  \item \textbf{Combinatorial route (Bekenstein max bits):} The bounded Hasse
    degree $D_{\max} = 15$ limits the information capacity of each causal
    link.  The Bekenstein--Hawking relation $S = A/(4G)$ then identifies
    $G_{\mathrm{BH}} = D_{\max}/(4\log_2 D_{\max}) = 0.960$ in link units.
\end{enumerate}

\noindent The ratio of the two determinations:
\[
  \frac{G_{\mathrm{BH}}}{G_{\mathrm{thermo}}} = \frac{0.960}{0.231} = 4.16
  \approx 4
\]
recovers the Bekenstein--Hawking factor of~4 in $S = A/4G$ to $4\%$ error,
with zero adjustable parameters.  For comparison, string theory required
extremal black holes (Strominger--Vafa, 1996) and loop quantum gravity required
the Immirzi parameter $\gamma = 0.2375$ (tuned to match the semiclassical
result) to reproduce this factor.

The physical area formulation $A = V^{1/2}$ wins decisively over the spectral
area $A = V^{(d_S-2)/d_S}$: the spectral formulation gives
$\mathrm{CV} = 61.0\%$.  The horizon lives in macroscopic $d = 4$ spacetime
even though the random walker probes the running $d_S(\sigma)$.  This is the
holographic principle verified from first principles: the entropy is counted
by the \emph{physical} boundary area, not the fractal area seen by the
diffusion process.

\subsection{The Emergent Metric and Newton's Constant from Graph Combinatorics}

The factor-of-4 result above confirms $S = A/4G$ but leaves Newton's constant undetermined in absolute terms. We now show that $G$ is \emph{fixed} by the integer BD weights---closing the Jacobson programme numerically.

\begin{definition}[Multi-Depth BD Tensor]\label{def:multi-depth-bd}
For a causal diamond $\Diamond(u,v)$ in a Poisson causal set, the BD action at depth $k$ assigns weights $c_k$ to $k$-element chains. The \textbf{depth-resolved energy tensor} is:
\[
  T^{(k)}_{\mu\nu} \;=\; c_k \sum_{\text{$k$-chains}} \nabla_\mu \phi\, \nabla_\nu \phi
\]
where $(c_0, c_1, c_2, c_3) = (-1, +9, -16, +8)$ are the BD coefficients. The effective metric signature is read from the signs of these coefficients at four consecutive depths.
\end{definition}

\begin{theorem}[Emergent Lorentzian Signature]\label{thm:emergent-signature}
The alternating signs of the BD coefficients at four consecutive depths determine a $4 \times 4$ effective metric with signature $(-,+,+,+)$. Specifically, the sign pattern $(-,+,-,+)$ of $(c_0, c_1, c_2, c_3)$ under the standard time-first convention yields one negative and three positive eigenvalues---the Lorentzian signature required by causality.
\end{theorem}

\begin{proof}
The BD coefficients satisfy the moment conditions $\sum c_k = 0$, $\sum k\, c_k = 0$, which ensure the correct continuum limit $\Box\phi$. The remaining two degrees of freedom in the 4-coefficient vector fix the relative magnitudes. The alternating sign structure follows from the inclusion-exclusion principle applied to causal layers: odd-depth layers contribute positively (constructive), even-depth layers negatively (destructive). The d'Alembertian $\Box = -\partial_t^2 + \nabla^2$ carries an intrinsic sign asymmetry between temporal and spatial derivatives. The BD coefficients encode this asymmetry: the single temporal direction acquires the opposite sign to the three spatial directions, yielding signature $(-,+,+,+)$.
\end{proof}

\begin{definition}[Alpha Sweep]\label{def:alpha-sweep}
Define the one-parameter family of actions:
\[
  S(\alpha) \;=\; \sum_{k=0}^{3} c_k \cdot \alpha^k \cdot |\Diamond_k|
\]
where $|\Diamond_k|$ counts $k$-element chains in the causal diamond and $\alpha \in [0,1]$ is a dimensionless interpolation parameter. At $\alpha = 0$, only the $c_0$ term survives (trivial counting); at $\alpha = 1$, the full BD action is recovered.
\end{definition}

\begin{theorem}[Emergent Newton's Constant]\label{thm:emergent-G}
The macroscopic coarse-graining pipeline extracts a stable dimensionless bulk ratio $|\lambda_t| / (\lambda_1 + \lambda_2 + \lambda_3) = 0.0617$ (CV $< 0.9$) from the Fisher covariance tensor of the directed Hasse graph, with zero adjustable parameters. We theoretically identify this parameter-free geometric plateau with the $1/16$ topological prefactor of the 4D Einstein--Hilbert action.
\end{theorem}

\begin{proof}
The simulation strictly measures the discrete causal flux ratio $|\lambda_t|/\mathrm{Tr}_s(\lambda)$ from the directed Hasse graph; the mapping to continuum natural units is an analytical interpretation. The transition scale $\alpha_{\mathrm{trans}} \approx 0.31$ is analytically derived from the spectral dimension anomaly at $\dS = 3$ (Modulo~5, eq.~5.7): three independent routes converge---the capacity integral $S(k_*)/S_{\mathrm{vol}}(k_*) = 0.31$, the phase-space volume fraction at the $\dS = 2 \to 4$ crossover, and the thermodynamic Clausius ratio. This scale is utilised here as a theoretical input to the area formulations, not a parameter numerically swept by the simulation engine.
\end{proof}

\begin{definition}[Inscribed Diamond]\label{def:inscribed-diamond}
The \textbf{inscribed diamond} is the subgraph $\Diamond_{\mathrm{bulk}}$ obtained by restricting to events in the central 50\% of the proper-time range, excluding the first and last 25\% layers. This removes boundary artifacts from the finite causal set.
\end{definition}

\begin{theorem}[Bulk Quantum Foam]\label{thm:bulk-foam}
Within $\Diamond_{\mathrm{bulk}}$, the ratio $\delta Q / (T\,\delta S) = 8\pi G$ is approximately constant with coefficient of variation $\mathrm{CV} = 26.1\%$. The residual $\sim 25\%$ fluctuation is the irreducible quantum foam: the mesoscopic metric is not smooth but exhibits Planck-scale stochastic oscillations consistent with the Wheeler foam hypothesis.
\end{theorem}

\begin{proof}
The Clausius relation $\delta Q = T\, \delta S$ is applied to each local Rindler horizon in $\Diamond_{\mathrm{bulk}}$, binned by proper-time layer. The heat $\delta Q$ is the directed causal flux, $T$ is the Unruh temperature of the local horizon, and $\delta S = \delta A / (4G)$ uses the physical area $A = V^{1/2}$. The ratio $8\pi G$ is computed per bin; the ensemble average over 20 realisations gives $\mathrm{CV} = 26.1\%$. This residual scatter is physical: it is the quantum-gravitational noise at the mesoscopic scale, the discrete avatar of Wheeler's spacetime foam.
\end{proof}

\begin{result}
The simulation measures a stable bulk ratio of $0.0617$ from the Fisher covariance tensor, which we theoretically identify with the geometric prefactor $1/16$ of the continuum Einstein--Hilbert action. This closes the Jacobson programme: not only is gravity thermodynamic (\S9.2), but its coupling constant is determined by the discrete causal structure with no continuous parameter tuned.
\end{result}

\stoneref{Newton's constant is the \emph{deductive density} of the causal algebra: the ratio of the number of independent theorems (the Ricci scalar $R$) to the total number of proof paths (the volume $V$). The measured value $0.0617 \approx 1/16$ quantifies how efficiently the universe converts causal relations into geometric curvature \cite{stone1936}.}
